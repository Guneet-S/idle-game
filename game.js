...
function createFreshState() {
  ...
  return {
    ...
    stats: {
      ...
      totalClicks: 0,
      clickPlasma: 0,
      autoPlasma: 0,    // FIX 5: separate auto-clicker tracking
      genPlasma: 0,
      startTime: Date.now(),
    },
  };
...
function getGenOutput(def) {
  ...
  const shardBonus = 1 + game.cosmicShards * 0.05; // 5% per held shard
...
// FIX 3: O(1) geometric series formula instead of O(n) loop
function buyMaxGenerator(id) {
  const def = GENERATOR_DEFS.find(g => g.id === id);
  const currentCount = game.generators[def.id];
  const C0 = def.baseCost * Math.pow(def.costMult, currentCount); // cost of the very next unit
  const r = def.costMult;

  // Max affordable n: floor( log(plasma*(r-1)/C0 + 1) / log(r) )
  const n = Math.floor(Math.log(game.plasma * (r - 1) / C0 + 1) / Math.log(r));
  if (n <= 0) return;

  // Total cost of buying n units: C0 * (r^n - 1) / (r - 1)
...
function doPrestige() {
  ...
  const clickPlasma    = game.stats.clickPlasma;      // FIX 1: persist clickPlasma through prestige
  const autoPlasma     = game.stats.autoPlasma || 0;  // FIX 5: persist autoPlasma through prestige
  ...
  game.stats.clickPlasma   = clickPlasma;   // FIX 1: restore
  game.stats.autoPlasma    = autoPlasma;    // FIX 5: restore
...
function updateUI() {
  ...
  // FIX 4: Show shard holding bonus in prestige tab
  ...
  document.getElementById('stat-auto-plasma').textContent  = fmt(game.stats.autoPlasma || 0); // FIX 5
...
function tick() {
  ...
  // FIX 5: Auto-clicker goes into autoPlasma, not clickPlasma
...
// ─── Save / Load ───
// FIX 6: genMultipliers and clickMultiplier removed — both are fully recomputed on load
function saveGame() {
  const data = {
    plasma: game.plasma,
...
function loadGame() {
  ...
  try {
    ...
    // Merge saved data — skip derived fields that are recomputed below
    Object.keys(data).forEach(key => {
      if (key === 'generators' || key === 'stats' || key === 'shardUpgrades') {
        Object.assign(game[key], data[key]);
      } else if (key !== 'savedAt' && key !== 'genMultipliers' && key !== 'clickMultiplier') {
        game[key] = data[key];
      }
    });

    // Re-apply upgrade effects fresh (recomputes clickMultiplier, genMultipliers, globalGenMultiplier)
    ...
    // FIX 2: Offline production with 12-hour cap
    if (data.savedAt) {
      const MAX_OFFLINE = 12 * 60 * 60; // 43200 seconds
      const offlineSecs = Math.min((Date.now() - data.savedAt) / 1000, MAX_OFFLINE);
      if (offlineSecs > 2) {
        const offlinePlasma = getTotalRate() * offlineSecs * 0.5; // 50% efficiency
        if (offlinePlasma > 0) {
          game.plasma += offlinePlasma;
          game.totalPlasmaThisRun += offlinePlasma;
